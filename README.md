...
- **`allowRoles`**: Restricts access based on role
- **`validators`**: Validates request body, returns structured errors
...
#### Use Cases
- **Admin Dashboard**: `GET /api/users?role=teamLead` (Fetch all team leads for assignment)
- **Team Lead Dashboard**: `GET /api/users` (Fetch only team members created by that team lead)
...
1. **Bugger** → Create Incident
2. → **Admin** assigns Team Lead
3. → **Team Lead** assigns Responders
4. → **Responders** update timeline
5. → **Status** updated → Incident resolved
...
- Example: Timeline updates allowed only if user is assigned to that incident
...
- [x] Incident APIs implemented
- [x] Timeline APIs implemented
- [x] Resource-level access control implemented
- [x] Indexing applied
